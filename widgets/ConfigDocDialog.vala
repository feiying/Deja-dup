...
public class ConfigDocDialog : Gtk.Dialog {
	...
	public ConfigDocDialog () {
		this.title = _("Documents and Data");
		this.border_width = 5;
	...
	private void create_widgets () {
		...
		Gtk.Label label = new Gtk.Label.with_mnemonic (_("Documents and Data"));
		button = new Gtk.Switch();
		Gtk.Label description = new Gtk.Label.with_mnemonic (_("Automaticlly upload Document Data and send them to all of you devices."));
	...
	private void handle_toggled() {
		var set = DejaDup.get_settings(APPS_ROOT+".document-data");
                set.set_boolean(BACKUP_KEY, button.get_active());
		doc_toggled(button.get_active());
